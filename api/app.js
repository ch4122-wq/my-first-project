...
function gate(req, res, origin) {
  ...
  return send(res, 200, 'The Members’ Corner', `<header><span class="label">The members’ corner</span><h1>A seat at<br><em>the little table.</em></h1><p>Come in for a slower moment. Your members-only coffee ritual is waiting.</p></header><div class="layout"><section class="panel"><span class="tag">Members only · Sign in required</span><h2>A little more to savor.</h2><p>Sign in with Google to rate captions, explore our tasting guide, and unlock a five-stop café itinerary.</p>${google}<p class="detail">We use your Google name and email to create your account. Your Google password stays with Google.</p></section><aside class="preview"><span class="lock" aria-hidden="true">✳</span><h2>Your next lovely pause</h2><p>A thoughtful coffee ritual, a fresh route through the collection, and a reason to linger a little longer.</p><span class="label">Unlock with your Google account</span></aside></div>`, !!config.googleClientId);
}
export async function handle(req, res, { makeClient = client } = {}) {
  ...
  if(route !== 'members' || !['GET','POST'].includes(req.method)) return send(res,404,'Not found','<h1>This page isn’t here.</h1><a href="/">Back to the collection</a>');
  const supabase = makeClient(req,res);
  ...
  const {data:captions,error:captionsError} = await supabase.from('captions').select('id,content').order('created_at',{ascending:true}).limit(50);
  const captionSection = `<section aria-labelledby="captions-title"><header><span class="label">The caption club</span><h2 id="captions-title" style="font:40px Georgia,serif">A little coffee. A little comedy.</h2><p>Which lines deserve another round? Give each caption an upvote or downvote. One vote per caption.</p></header>${notice ? `<p role="status" class="panel">${e(notice)}</p>`:''}${captionsError ? '<p role="alert">Captions could not be loaded. Please refresh to try again.</p>' : !captions?.length ? '<p>New captions are brewing. Check back soon.</p>' : `<div class="layout">${captions.map((caption,i)=>`<article class="panel"><span class="label">Caption ${String(i+1).padStart(2,'0')}</span><p style="font:24px/1.5 Georgia,serif;color:#2c302b">${e(caption.content)}</p><form action="/members" method="post"><input type="hidden" name="csrf" value="${csrf}"><input type="hidden" name="caption_id" value="${e(caption.id)}"><button type="submit" name="vote" value="1" aria-label="Upvote caption ${i+1}">↑ Upvote</button> <button type="submit" name="vote" value="-1" aria-label="Downvote caption ${i+1}" style="background:#e9ecdf;color:#435342">↓ Downvote</button></form></article>`).join('')}</div>`}</section>`;

  return send(res,200,'Welcome to the Members’ Corner',`<header><span class="label">The members’ corner · Unlocked</span><h1>Your next<br><em>lovely pause.</em></h1><div class="welcome"><p class="account">Welcome, ${e(user.user_metadata?.full_name || user.email || 'coffee friend')}.</p><form action="/auth/signout" method="post"><button type="submit">Sign out</button></form></div></header>${captionSection}<header><span class="label">Stay for a little longer</span></header><div class="layout"><section class="panel"><span class="tag">Your members-only tasting guide</span><h2>The three-sip ritual.</h2><ol class="list"><li><strong>01 · Notice the aroma</strong><span>Take a breath before the first sip. What comes to mind: cocoa, citrus, or toasted nuts?</span></li><li><strong>02 · Let it settle</strong><span>Try a second sip as the cup cools. Pay attention to sweetness, texture, and the finish.</span></li><li><strong>03 · Make a moment of it</strong><span>Put your phone away for five minutes. Leave a little room for an unhurried conversation.</span></li></ol></section><aside class="preview"><span class="label">The five-stop daydream</span><h2>Take the scenic route.</h2><ol class="list"><li>Sunday Window — a soft start</li><li>Moss &amp; Mug — a leafy interlude</li><li>Blue Hour — a riverside reset</li><li>Little Ember — something sweet</li><li>Paper Moon — time to create</li></ol><p class="detail">An imagined itinerary through our fictional café collection.</p><a href="/">Revisit the collection ↗</a></aside></div>`);
}
